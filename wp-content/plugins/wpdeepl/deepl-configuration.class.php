...
class DeepLConfiguration {
	...
	static function getActiveGlossaryFor( $source_language, $target_language) {
		$source = strtolower( substr( $source_language, 0, 2 ) );
		$target = strtolower( substr( $target_language, 0, 2 ) );
		$active_glossary_id = get_option( 'wpdeepl_glossary_' . $source. '_' . $target );
