...
if ( ! class_exists( 'Redux_Framework_Plugin', false ) ) {
	...
	class Redux_Framework_Plugin {
		...
		public static function instance(): ?Redux_Framework_Plugin {
			...
			if ( function_exists( 'get_plugin_data' ) && file_exists( $path ) ) {
				...
				if ( is_plugin_active( 'redux-framework/redux-framework.php' ) && true === $res ) {
					echo '<div class="error"><p>' . esc_html__( 'Redux Framework version 4 is activated but not loaded. Redux Framework version 3 is still installed and activated.  Please deactivate Redux Framework version 3.', 'bighearts-core' ) . '</p></div>'; // phpcs:ignore WordPress.Security.EscapeOutput
					return null;
				}
			}
		...
		private static function single_activate() {
			...
			$notices[] = esc_html__( 'Redux Framework has an embedded demo.', 'bighearts-core' ) . ' <a href="./plugins.php?redux-framework-plugin=demo&nonce=' . $nonce . '">' . esc_html__( 'Click here to activate the sample config file.', 'bighearts-core' ) . '</a>';
		...
		public function plugin_metalinks( array $links, string $file ): array {
			if ( strpos( $file, 'redux-framework.php' ) !== false && is_plugin_active( $file ) ) {
				$links[] = '<a href="' . esc_url( admin_url( add_query_arg( array( 'page' => 'redux-framework' ), 'options-general.php' ) ) ) . '">' . esc_html__( 'What is this?', 'bighearts-core' ) . '</a>';
			}
