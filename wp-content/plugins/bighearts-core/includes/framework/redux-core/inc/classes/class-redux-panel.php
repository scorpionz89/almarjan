...
if ( ! class_exists( 'Redux_Panel', false ) ) {
	...
	class Redux_Panel {
		...
		private function panel_template() {
			...
			// Do we support JS?
			echo '<noscript><div class="no-js">' . esc_html__( 'Warning- This options panel will not work properly without javascript!', 'bighearts-core' ) . '</div></noscript>';
			...
			echo '<div id="redux-dialog-confirm" title="' . esc_html__( 'Submit Support Information?', 'bighearts-core' ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'redux_submit_support' ) ) . '">';
			// translators: %s = WP Site Health Screen URL.
			echo '<p><span class="ui-icon ui-icon-alert" style="float:left; margin:12px 12px 20px 0;"></span>' . sprintf( esc_html__( 'Redux will send the debug information found on the %s and your Redux config to our secure server.  Are you ready to proceed?', 'bighearts-core' ), '<a href="' . esc_url( admin_url( 'site-health.php?tab=debug' ) ) . '" target="_blank">' . esc_html__( 'WordPress Site Health screen', 'bighearts-core' ) . '</a>' ) . '</p>';
			echo '</div>';
		...
		public function notification_bar() {
			if ( isset( $this->parent->transients['last_save_mode'] ) ) {
				...
				if ( 'import' === $this->parent->transients['last_save_mode'] ) {
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'bighearts-core' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", esc_html__( 'All Defaults Restored!', 'bighearts-core' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-defaults-section-text-{$this->parent->args['opt_name']}", esc_html__( 'Section Defaults Restored!', 'bighearts-core' ) ) ) . '</strong>';
					echo '</div>';
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					echo '<strong>' . esc_html( apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Saved!', 'bighearts-core' ) ) ) . '</strong>';
					echo '</div>';
				}
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			echo '<strong>' . esc_html( apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'bighearts-core' ) ) ) . '</strong>';
			echo '</div>';
			...
			echo '<strong>';
			echo '<span></span> ' . esc_html__( 'error(s) were found!', 'bighearts-core' );
			echo '</strong>';
			echo '</div>';
			...
			echo '<strong>';
			echo '<span></span> ' . esc_html__( 'warning(s) were found!', 'bighearts-core' );
			echo '</strong>';
			echo '</div>';
		}
		...
		public function get_template( string $file ) {
			...
			if ( file_exists( $path ) ) {
				if ( is_readable( $path ) ) {
					...
					echo '<div class="error"><p>' . sprintf( esc_html__( 'Redux Panel Template %1$s cannot be read. Please check the permissions for this file.', 'bighearts-core' ), '<code>' . esc_html( $path ) . '</code>' ) . '</p></div>';
				}
			} else {
				if ( file_exists( Redux_Core::$dir . 'templates/panel/' . $file ) ) {
					...
					// translators: %1$s: template path.
					echo '<div class="error"><p>' . sprintf( esc_html__( 'Redux Panel Template %1$s does not exist. Please reinstall Redux to replace this file.', 'bighearts-core' ), '<code>' . esc_html( $path ) . '</code>' ) . '</p></div>';
				}
			}
		}
				...
				if ( $developer_theme_file ) {
					...
					if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) /* && isset( $this->parent->args['dev_mode'] ) && ! empty( $this->parent->args['dev_mode'] ) */ ) {
						...
						<div id="message" class="error redux-message" style="display:block!important">
							<p>
								<strong><?php esc_html_e( 'Your panel has bundled copies of Redux Framework template files that are outdated!', 'bighearts-core' ); ?></strong>&nbsp;&nbsp;<?php esc_html_e( 'Please ask the author of this theme to update them as functionality issues could arise.', 'bighearts-core' ); ?>
							</p>
						</div>
						<?php
