	...
	class Redux_Connection_Banner {
		...
		public function admin_ajax() {
			...
			if ( empty( $nonce ) || ! wp_verify_nonce( $nonce, $this->nonce ) ) {
				die( __( 'Security check failed.', 'bighearts-core' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		...
		private function install_extendify(): bool {
			...
			if ( empty( $download_link ) ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Error: Install URL for Extendify could not be located.', 'bighearts-core' ),
					)
				);
			...
			if ( true !== $install ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Install process for Extendify failed.', 'bighearts-core' ),
					)
				);
			...
			if ( is_wp_error( $activate ) ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Extendify activation failed.', 'bighearts-core' ),
					)
				);
			}
			...
			echo wp_json_encode(
				array(
					'type' => 'success',
					'msg'  => esc_html__( 'Extendify installed and activated.', 'bighearts-core' ),
				)
			);
			...
			<div id="redux-connect-message" class="updated redux-banner-container" data-nonce="<?php echo wp_create_nonce( $this->nonce ); ?>">
				<div class="redux-banner-inner-container">
					<a href="<?php echo esc_url( $urls['dismiss'] ); ?>" data-url="<?php echo admin_url( 'admin-ajax.php' ); ?>"
					   ...
					   title="<?php esc_attr_e( 'Dismiss this notice', 'bighearts-core' ); ?>" data-activate="false">
					</a>
					...
					<div class="redux-banner-content-container">
						...
						<div class="redux-banner-slide redux-banner-slide-one redux-slide-is-active">
							...
							<div class="redux-banner-slide-text">
								<h2><?php esc_html_e( 'Action needed to continue using the template library.', 'bighearts-core' ); ?></h2>
								<p>
									<?php
									esc_html_e(
								...
								<div class="redux-banner-button-container">
									...
									<a href="<?php echo esc_url( $urls['register'] ); ?>" data-url="<?php echo admin_url( 'admin-ajax.php' ); ?>" data-activate="main_banner"
									   class="button button-primary button-large redux-alt-connect-button redux-connection-banner-action">
										<?php esc_html_e( 'Install and Activate Extendify', 'bighearts-core' ); ?>
									</a>
								</div>
