...
( function( $ ) {
	...
	redux.field_objects.social_profiles.resetItem = function( el, itemID ) {
		...
		var defaultTextColorHover       = reduxSocialDefaults[itemID].color_hover;
		var defaultBackgroundColorHover = reduxSocialDefaults[itemID].background_hover;

		el.find( '.redux-social-profiles-color-picker-' + defItemID + '.text' ).spectrum( 'set', defaultTextColor );
		el.find( '.redux-social-profiles-color-picker-' + defItemID + '.background' ).spectrum( 'set', defaultBackgroundColor );
		el.find( '.redux-social-profiles-color-picker-' + defItemID + '.color_hover' ).spectrum( 'set', defaultTextColorHover );
		el.find( '.redux-social-profiles-color-picker-' + defItemID + '.background_hover' ).spectrum( 'set', defaultBackgroundColorHover );

		redux.field_objects.social_profiles.updateDataString( el, defItemID, 'color', defaultTextColor );
		redux.field_objects.social_profiles.updateDataString( el, defItemID, 'background', defaultBackgroundColor );
		redux.field_objects.social_profiles.updateDataString( el, defItemID, 'color_hover', defaultTextColorHover );
		redux.field_objects.social_profiles.updateDataString( el, defItemID, 'background_hover', defaultBackgroundColorHover );

		redux.field_objects.social_profiles.updatePreview( el, defItemID );
	};
	...
	redux.field_objects.social_profiles.updatePreview = function( el, itemID ) {
		...
		var textHoverColorInput       = redux.field_objects.social_profiles.valueFromDataString( el, itemID, 'color_hover' );
		var backgroundHoverColorInput = redux.field_objects.social_profiles.valueFromDataString( el, itemID, 'background_hover' );

		var defIconID = itemID.replace(/[^0-9.]/g, '');
		var icon   = reduxSocialDefaults[defIconID].icon;
		...
		symbol.on({
			mouseenter: function () {
				symbol.css( 'background-color', backgroundHoverColorInput );
				symbol.css( 'color', textHoverColorInput );
			},
			mouseleave: function () {
				symbol.css( 'background-color', backgroundColorInput );
				symbol.css( 'color', textColorInput );
						...
						el.find( '.redux-social-profiles-color-picker-' + key ).spectrum(
							{
								...
								change: function( color ) {
									...
									if ( $( this ).hasClass( 'text' ) ) {
										className = 'color';
									}else if ($( this ).hasClass( 'color_hover' )) {
										className = 'color_hover';
									}else if ($( this ).hasClass( 'background_hover' )) {
